...
Esta versão corrige os últimos avisos do Clippy atual no runner Windows: importações específicas de plataforma no launcher agora estão sob `cfg`, o backend de registro unitário é instanciado sem `Default`, e o reporter nulo é restrito aos testes.
